...
async function confirmarOperario() {
    ...
    // El login manual sustituye al de tarjeta: liberar el lector.
    _pararLoginPoll();
...
// El PC pide un login contra un carro; el servidor da un código que se ve en la
// pantalla del carro. El operario pasa su tarjeta allí y la petición se resuelve.

async function iniciarLoginTarjeta() {
    _pararLoginPoll();
    _setLoginMsg('Buscando lector…', '#334155');
    _mostrarCodigoLogin(null);

    let lectores = [];
    try {
        const d = await (await fetch('/api/esp32/lectores')).json();
        if (d.success) lectores = d.lectores || [];
    } catch (_) { /* sin red: se ofrece el manual */ }

    const selWrap = document.getElementById('login-selector-lector');
    const selLector = document.getElementById('login-lector');

    if (lectores.length === 0) {
        if (selWrap) selWrap.classList.add('hidden');
        _setLoginMsg('No hay ningún lector disponible ahora mismo. Entra manualmente abajo.', '#b45309');
    ...
    if (lectores.length === 1) {
        if (selWrap) selWrap.classList.add('hidden');
        loginCarroSel = lectores[0].carro;
    } else {
        if (selWrap) selWrap.classList.remove('hidden');
        if (selLector) {
            selLector.innerHTML = lectores.map(l =>
                `<option value="${l.carro}">Carro ${l.carro}${l.nfc === 'ok' ? '' : ' (lector sin confirmar)'}</option>`
            ).join('');
            loginCarroSel = selLector.value;
        }
    }
    _solicitarLoginTarjeta();
}

function cambiarLectorLogin() {
    const selLector = document.getElementById('login-lector');
    if (!selLector) return;
    _cancelarLoginRequest();
    loginCarroSel = selLector.value;
...
async function _solicitarLoginTarjeta() {
    if (!loginCarroSel || operarioActual) return;
    _pararLoginPoll();
    _setLoginMsg('Preparando el lector…', '#334155');
    try {
        const r = await fetch('/api/operarios/login/solicitar', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ carro: loginCarroSel })
        });
        const d = await r.json();
        if (r.status === 409 || d.ocupado) {
            _setLoginMsg('El lector está ocupado por otro login. Reintentando…', '#b45309');
            _mostrarCodigoLogin(null);
            setTimeout(() => { if (!operarioActual) _solicitarLoginTarjeta(); }, 4000);
            return;
        }
        if (!d.success) {
            _setLoginMsg(d.error || 'No se pudo preparar el login.', '#dc2626');
            return;
        }
        loginRequestId = d.request_id;
        _setLoginMsg(`Pasa tu tarjeta por el lector del carro ${loginCarroSel}.`, '#334155');
        _mostrarCodigoLogin(d.codigo);
        _iniciarLoginPoll();
    } catch (_) {
        _setLoginMsg('Sin conexión con el servidor. Reintentando…', '#dc2626');
        setTimeout(() => { if (!operarioActual) _solicitarLoginTarjeta(); }, 4000);
    }
}

function _iniciarLoginPoll() {
    _pararLoginPoll();
    _loginPollTimer = setInterval(async () => {
        if (!loginRequestId || operarioActual) { _pararLoginPoll(); return; }
        try {
            const d = await (await fetch('/api/operarios/login/solicitar/estado', {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ request_id: loginRequestId })
            })).json();
            if (d.estado === 'resuelto') {
                _pararLoginPoll();
                operarioLoginId = d.login_id;
                loginRequestId = null;
                _setLoginMsg(`¡Hola, ${d.operario}!`, '#16a34a');
                _iniciarLatidoOperario();
                sessionStorage.setItem('operario_actual', d.operario);
                _activarOperario(d.operario);
            } else if (d.estado === 'rechazado') {
                _setLoginMsg(d.error || 'No puedes entrar ahora mismo.', '#dc2626');
                _mostrarCodigoLogin(null);
                _pararLoginPoll();
                loginRequestId = null;
                setTimeout(() => { if (!operarioActual) _solicitarLoginTarjeta(); }, 5000);
            } else if (d.estado === 'expirado' || d.estado === 'cancelado') {
                _pararLoginPoll();
                loginRequestId = null;
                if (!operarioActual) _solicitarLoginTarjeta();
            }
        } catch (_) { /* reintenta en el próximo sondeo */ }
    }, 2000);
}

function _pararLoginPoll() {
    if (_loginPollTimer) { clearInterval(_loginPollTimer); _loginPollTimer = null; }
}

function _cancelarLoginRequest() {
    if (!loginRequestId) return;
    const id = loginRequestId;
    loginRequestId = null;
    fetch('/api/operarios/login/solicitar/cancelar', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ request_id: id })
    }).catch(() => {});
}

function _setLoginMsg(txt, color) {
    const el = document.getElementById('login-tarjeta-msg');
    if (el) { el.textContent = txt; el.style.color = color || '#334155'; }
}

function _mostrarCodigoLogin(codigo) {
    const wrap = document.getElementById('login-tarjeta-codigo');
    const val = document.getElementById('login-codigo-valor');
    if (!wrap || !val) return;
    if (codigo) { val.textContent = codigo; wrap.classList.remove('hidden'); }
